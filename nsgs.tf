...
resource "oci_core_network_security_group_security_rule" "ingress" {
  ...
  source      = each.value.src_type != "NETWORK_SECURITY_GROUP" ? (each.value.src_type != "SERVICE_CIDR_BLOCK" ? each.value.src : contains(keys(local.oci_services_details), each.value.src) ? local.oci_services_details[each.value.src].cidr_block : each.value.src) : oci_core_network_security_group.these[each.value.src].id
...
resource "oci_core_network_security_group_security_rule" "egress" {
  ...
  destination      = each.value.dst_type != "NETWORK_SECURITY_GROUP" ? (each.value.dst_type != "SERVICE_CIDR_BLOCK" ? each.value.dst : contains(keys(local.oci_services_details), each.value.dst) ? local.oci_services_details[each.value.dst].cidr_block : each.value.dst) : oci_core_network_security_group.these[each.value.dst].id
