...
# PROCESSED INPUT
locals {
  one_dimension_processed_security_lists = local.one_dimension_processed_vcns != null ? {
    for flat_security_list in flatten([
      for vcn_key, vcn_value in local.one_dimension_processed_vcns :
      vcn_value.security_lists != null ? length(vcn_value.security_lists) > 0 ? [
        for sec_list_key, sec_list_value in vcn_value.security_lists : {
          compartment_id          = sec_list_value.compartment_id != null ? sec_list_value.compartment_id : vcn_value.category_compartment_id != null ? vcn_value.category_compartment_id : vcn_value.default_compartment_id != null ? vcn_value.default_compartment_id : null
          default_compartment_id  = vcn_value.default_compartment_id
          category_compartment_id = vcn_value.category_compartment_id
          defined_tags            = merge(sec_list_value.defined_tags, vcn_value.category_defined_tags, vcn_value.default_defined_tags)
          default_defined_tags    = vcn_value.default_defined_tags
          category_defined_tags   = vcn_value.category_defined_tags
          freeform_tags           = merge(sec_list_value.freeform_tags, vcn_value.category_freeform_tags, vcn_value.default_freeform_tags)
          ...
          security_list_key              = sec_list_key
        }
      ] : [] : []
    ]) : flat_security_list.security_list_key => flat_security_list
  } : null


  one_dimension_processed_injected_security_lists = local.one_dimension_processed_existing_vcns != null ? {
    for flat_security_list in flatten([
  ...
  merged_one_dimension_processed_security_lists = merge(local.one_dimension_processed_security_lists, local.one_dimension_processed_injected_security_lists)
  ...
  provisioned_security_lists = {
    for sec_list_key, sec_list_value in oci_core_security_list.these : sec_list_key => {
      ...
      vcn_id                         = sec_list_value.vcn_id
      vcn_key                        = local.merged_one_dimension_processed_security_lists[sec_list_key].vcn_key
      vcn_name                       = local.merged_one_dimension_processed_security_lists[sec_list_key].vcn_name
      network_configuration_category = local.merged_one_dimension_processed_security_lists[sec_list_key].network_configuration_category
...
resource "oci_core_security_list" "these" {
  for_each = local.merged_one_dimension_processed_security_lists
